...
from configuration import (
    sql_file_path,
)  # Assuming there's a configuration file with DB path
...
class ScoreManager:
    ...
    def add_or_update_score(self, slack_user_id, category, points=1):
        ...
        try:
            user_score = (
                session.query(UserScore).filter_by(slack_user_id=slack_user_id).first()
            )
            if not user_score:
        ...
        try:
            if category == "seeker":
                return (
                    ...
                    .order_by(UserScore.seeker_score.desc())
                    .limit(top_n)
                    .all()
                )
            elif category == "revealer":
                return (
                    session.query(UserScore)
                    .order_by(UserScore.revealer_score.desc())
                    .limit(top_n)
                    .all()
                )
            elif category == "luminary":
                return (
                    session.query(UserScore)
                    .order_by(UserScore.luminary_score.desc())
                    .limit(top_n)
                    .all()
                )
            else:
