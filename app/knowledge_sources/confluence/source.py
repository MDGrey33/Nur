...
from app.knowledge_sources.confluence.models.knowledge_source_item import (
    KnowledgeSourceItem,
)
...
class ConfluenceSource:
    ...
    def process(self):
        ...
        for space_page in space_pages:
            space_page_data = self.confluence.get_page_by_id(
                space_page["id"], expand="body.storage,history,version"
            )
            ...
            page_content = self.strip_html_tags(
                space_page_data.get("body", {}).get("storage", {}).get("value", "")
            )
            page_comments = self.confluence.get_page_child_by_type(
                space_page["id"], "comment"
            )
