...
class QAInteractionManager:
    ...
    def add_question_and_answer(
        ...
        try:
            serialized_answer = (
                json.dumps(answer.__dict__) if not isinstance(answer, str) else answer
            )
    ...
    def add_comment_to_interaction(self, thread_id, comment):
        ...
        try:
            interaction = (
                session.query(QAInteractions).filter_by(thread_id=thread_id).first()
            )
            if interaction:
    ...
    def get_interactions_by_interaction_ids(self, interaction_ids):
        ...
        try:
            ...
            return (
                session.query(QAInteractions)
                .filter(QAInteractions.interaction_id.in_(interaction_ids))
                .all()
            )
        except Exception as e:
            session.rollback()
    ...
    def is_message_processed(self, channel_id, message_ts):
        ...
        try:
            return (
                session.query(SlackMessageDeduplication)
                .filter_by(channel_id=channel_id, message_ts=message_ts)
                .first()
                is not None
            )
        finally:
