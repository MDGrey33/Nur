...
class EventConsumer:
    ...
    def add_question_and_response_to_database(
        self, question_event, response_text, assistant_thread_id
    ):
    ...
    def process_question(self, question_event: QuestionEvent):
        ...
        try:
            ...
            response_text, assistant_thread_id = query_assistant_with_context(
                question_event.text, context_page_ids, None
            )
        ...
        if response_text:
            ...
            try:
                ...
                try:
                    self.score_manager.add_or_update_score(
                        slack_user_id=question_event.user, category="seeker", points=1
                    )
                    print(f"Score updated for user {question_event.user}")
                except Exception as e:
                    print(f"Error updating score for user {question_event.user}: {e}")
                self.web_client.chat_postMessage(
                    channel=channel_id, text=response_text, thread_ts=message_ts
                )
    ...
    def process_feedback(self, feedback_event: FeedbackEvent):
        ...
        try:
            existing_interaction = (
                self.interaction_manager.get_interaction_by_thread_id(thread_ts)
            )
            assistant_thread_id = (
                existing_interaction.assistant_thread_id
                if existing_interaction
                else None
            )
        ...
        if existing_interaction:
            extended_context_query = self.generate_extended_context_query(
                existing_interaction, feedback_event.text
            )
            print(f"\n\nExtended context: {extended_context_query}\n\n")
            page_ids = retrieve_relevant_documents(extended_context_query)
            try:
                response_text, assistant_thread_id = query_assistant_with_context(
                    feedback_event.text, page_ids, assistant_thread_id
                )
            except Exception as e:
        ...
        if response_text:
            ...
            self.interaction_manager.add_comment_to_interaction(
                thread_id=thread_ts, comment=comment
            )
            print(
                f"Feedback appended to the interaction in the database: {feedback_event.dict()}\n"
            )
            self.web_client.chat_postMessage(
