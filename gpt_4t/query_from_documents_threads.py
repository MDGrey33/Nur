...
def get_response_from_gpt_4t(question, context):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": f"You will answer the following question with a summary, then provide a comprehensive answer, then provide the references aliasing them as Technical trace: including Space key, Page ID and Page title \nquestion: {question}\npages:{context}"
            }
        ],
        temperature=0.5,
        max_tokens=4095,
