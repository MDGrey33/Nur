...
class ConfluenceHelper:
    ...
    def get_page_by_id(self, page_id):
        return self.confluence.get_page_by_id(
            page_id, expand="body.storage,history,version"
        )
    ...
    def get_page_comments(self, page_id):
        ...
        def get_child_comment_ids_recursively(comment_id):
            child_comments = []
            immediate_children = self.confluence.get_page_child_by_type(
                comment_id, "comment"
            )
        ...
        for comment in all_comments:
            comment = self.confluence.get_page_by_id(
                comment["id"], expand="body.storage"
            )
            comment_content = (
