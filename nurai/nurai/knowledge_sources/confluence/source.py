...
from nurai.database.chroma import get_chroma
from nurai.knowledge_sources.confluence.confluence_helper import ConfluenceHelper
from openai import OpenAI

from nurai.knowledge_sources.models.knowledge_source import KnowledgeSource
from nurai.knowledge_sources.models.knowledge_source_item import KnowledgeSourceItem
from nurai.knowledge_sources.confluence.requests.confluence_options import (
    ConfluenceOptions,
)
from nurai.storage.storage_manager import StorageManager
...
class ConfluenceSource:
    ...
    def process(self):
        ...
        global_knowledge_collection = chroma_client.get_or_create_collection(
            name="global_knowledge_collection"
        )
        ...
        for space_page in space_pages:
            ...
            knowledge_source_item = KnowledgeSourceItem(
                ...
                created_at=datetime.strptime(
                    space_page_data["history"]["createdDate"], "%Y-%m-%dT%H:%M:%S.%fZ"
                ),
                updated_at=datetime.strptime(
                    space_page_data["version"]["when"], "%Y-%m-%dT%H:%M:%S.%fZ"
                ),
                file_identifier="confluence"
                + self.options.space_key
                + space_page_data["id"],
            )
            ...
            embedding_json = self.open_ai_client.embeddings.create(
                input=final_page_content, model="text-embedding-3-large"
            )
            ...
            self.storageManager.create_file(
                knowledge_source_item.file_identifier, final_page_content
            )
    ...
    def should_process(self, knowledge_source_item):
        return (
            knowledge_source_item.last_processed_at is None
            or knowledge_source_item.updated_at
            > knowledge_source_item.last_processed_at
        )
