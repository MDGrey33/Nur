...
async def start_chat_service(
    service_name: str = Query(
        "slack", description="The name of the chat service to start."
    )
):
    try:
...
async def fetch_chat_thread(
    service_name: str = Query(..., description="The name of the chat service."),
    channel: str = Query(..., description="The ID of the channel."),
    ts: str = Query(..., description="The thread timestamp."),
):
    try:
        chat_service: ChatServiceInterface = ChatServiceFactory.get_service(
            service_name
        )
        ...
        if messages is None:
            ...
            raise HTTPException(
                status_code=404, detail="Messages could not be fetched."
            )
        interaction_input = chat_service.transform_messages_to_interaction_input(
            messages, channel
        )
        if interaction_input is None:
            ...
            raise HTTPException(
                status_code=404,
                detail="Failed to transform messages into interaction input.",
            )
